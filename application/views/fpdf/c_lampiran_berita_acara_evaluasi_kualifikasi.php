...
//Header
		...
		$pdf->Cell(190,6,'LAMPIRAN BERITA ACARA EVALUASI KUALIFIKASI',0,2,'C');
		...
		$pdf->SetFont('Arial','B',12);
			for($i=0;$i<1;$i++){
			$pdf->Row(array('No','Dokumen Kualifikasi','Kelengkapan','Keterangan')); 
			}
		...
		for($i=0;$i<2;$i++){
		$n++;
			$pdf->Row(array($n,GenerateSentence(),'Ada','memenuhi syarat')); 
			}
		$pdf->Ln(5);
