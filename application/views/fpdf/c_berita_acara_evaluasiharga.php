...
//Header
		...
		$pdf->SetFont('Arial','B',12);
			for($i=0;$i<1;$i++){
			$pdf->Row(array('Satuan Kerja Biro Umum Sekretariat Jenderal Kementerian Kelautan dan Perikanan Tahun Anggaran XXXX','Berita Acara Evaluasi Harga')); 
			}
...
Tanggal : xx xxxxx xxxx')); 
		...
		$pdf->Ln(5);
		$pdf->MultiCell(190,5,'Pada hari ini, XXXX tanggal xxxxx xxxx bulan xxxxxx tahun xxxx, yang bertanda tangan dibawah ini Pejabat Pengadaan Barang / Jasa Satker Biro Umum telah melaksanakan evaluasi harga terhadap x (xxxx) perusahaan yang memasukkan dokumen penawaran, dengan hasil berikut :',0,'J');
		$pdf->Ln(5);
		...
		//header
			for($i=0;$i<1;$i++){
			$pdf->Row(array('No','Nama Perusahaan','Harga Penawaran','Harga Penawaran','Harga Terkoreksi')); 
			}
		...
		for($i=0;$i<2;$i++){
		$n++;
			$pdf->Row(array($n,GenerateSentence(),GenerateSentence(),GenerateSentence(),GenerateSentence())); 
			}
		$pdf->Ln(5);
