...
//Header
		...
		$pdf->SetFont('Arial','B',12);
			for($i=0;$i<1;$i++){
			$pdf->Row(array('Satuan Kerja Biro Umum Sekretariat Jenderal Kementerian Kelautan dan Perikanan Tahun Anggaran XXXX','Berita Acara Evaluasi Kualifikasi')); 
			}
...
Tanggal : xx xxxxx xxxx')); 
		...
		$pdf->Ln(5);
		$pdf->MultiCell(190,5,'Pada hari ini, XXXX tanggal xxxxx xxxx bulan xxxxxx tahun xxxx, yang bertanda tangan dibawah ini Pejabat Pengadaan Barang / Jasa Satker Biro Umum telah melaksanakan evaluasi kualifikasi terhadap x (xxxx) perusahaan yang memenuhi syarat evaluasi dokumen harga, dengan hasil sebagai berikut :',0,'J');
		$pdf->Ln(5);
		$w = array(10,110,35,35);
		$pdf->SetWidths($w);
		$pdf->SetAligns('C');
		//header
		$pdf->SetFont('Arial','B',12);
			for($i=0;$i<1;$i++){
			$pdf->Row(array('No','Dokumentasi Kualifikasi','Ada','Tidak Ada')); 
			}
		...
		$pdf->SetAligns('L');
		for($i=0;$i<9;$i++){
			$pdf->Row(array($huruf[$i],$dkul[$i],$pdf->Image('checkmark.jpeg', $pdf->GetX()+135, $pdf->GetY(), 4),'')); 
			}
		$pdf->Ln(5);
		
		$pdf->MultiCell(190,5,'Demikian Berita Acara Evaluasi Kualifikasi beserta lampirannya ini dibuat oleh Pejabat Pengadaan Barang / Jasa Satker Biro Umum Setjen KKP untuk selanjutnya akan dilakukan Klasifikasi dan negosiasi.',0,'J');
		$pdf->Ln(15);
		$pdf->Cell(100);
